...
export default function Sidebar({ isOpen, setIsOpen }) {
    ...
    // Check screen size - consider tablets too (not just mobile)
    useEffect(() => {
        const handleResize = () => {
            setIsMobileView(window.innerWidth < 1024); // Changed from 768 to 1024 to include tablets
        };
    ...
    return (
        <>
            {/* Mobile/tablet overlay - shown when sidebar is open on mobile/tablet */}
            {isMobileView && isOpen && (
                <div
            ...
            <div
                className={`${isMobileView
                    ? `fixed inset-y-0 left-0 z-30 w-64 transform ${isOpen ? 'translate-x-0' : '-translate-x-full'}`
                    : 'w-64'
                    } bg-white shadow-lg flex flex-col h-full overflow-hidden transition-transform duration-300 ease-in-out`}
            >
                ...
                <div className="px-6 py-4 border-b border-gray-200">
                    <div className="flex items-center justify-between">
                        ...
                        {isMobileView && (
                            <button
                                onClick={() => setIsOpen(false)}
                                className="lg:hidden rounded-md text-gray-500 hover:text-gray-800 focus:outline-none focus:ring-2 focus:ring-blue-500"
                            >
